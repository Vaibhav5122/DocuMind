        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground pt-0.5">
          ...
          <div className="flex items-center gap-1.5">
            {selectedDocIds.length > 0 ? (
              ...
              <button
                onClick={onSelectAll}
                className="text-[11px] text-primary hover:underline transition-colors font-medium cursor-pointer"
              >
                Select all
              </button>
            )}
...
function DocumentSidebarItem({
  ...
  return (
    <div
      onClick={onToggle}
      className={`group relative flex items-start gap-2.5 p-2.5 rounded-lg border transition-all cursor-pointer select-none text-left ${
        isSelected
          ? "bg-primary/10 border-primary/40 shadow-2xs"
          : "bg-card hover:bg-muted/50 border-border/60 hover:border-border"
      }`}
    >
      {/* Custom Checkbox */}
      <div
...
function StatusBadge({ status }: { status: DocumentStatus }) {
  if (status === "READY") {
    return (
      <span className="inline-flex items-center gap-0.5 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded-full shrink-0">
  ...
  if (status === "PROCESSING") {
    return (
      <span className="inline-flex items-center gap-1 text-[10px] font-medium text-amber-600 dark:text-amber-400 bg-amber-500/10 px-1.5 py-0.5 rounded-full shrink-0">
        <span className="h-2 w-2 rounded-full border-1 border-current border-t-transparent animate-spin" />
        Indexing
      </span>
    );
  }
  ...
  if (status === "UPLOADED") {
    return (
      <span className="inline-flex items-center gap-0.5 text-[10px] font-medium text-blue-600 dark:text-blue-400 bg-blue-500/10 px-1.5 py-0.5 rounded-full shrink-0">
  ...
  return (
    <span className="inline-flex items-center gap-0.5 text-[10px] font-medium text-rose-600 dark:text-rose-400 bg-rose-500/10 px-1.5 py-0.5 rounded-full shrink-0">
